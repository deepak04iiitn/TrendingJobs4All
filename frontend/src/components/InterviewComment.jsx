...
export default function InterviewComment({ comment, onLike, onEdit, onDelete }) {
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(`/backend/interviewComments/editComment/${comment._id}`, {
        method: 'PUT',
        headers: {
  ...
  const canModify = Boolean(currentUser && (currentUser._id === comment.userId || currentUser.isAdmin));
