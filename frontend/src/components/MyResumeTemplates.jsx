...
export default function MyResumeTemplates() {
  ...
  const handleDeleteTemplate = async () => {
    try {
        const response = await fetch(`/backend/resumeTemplates/delete/${selectedTemplateId}`, {
            method: 'DELETE',
        });
        ...
        setTemplates(prev => prev.filter(res => res._id !== selectedTemplateId));
    ...
    <div className="container mx-auto px-4 py-16">
      <div className="max-w-md mx-auto text-center bg-white shadow-xl rounded-2xl p-8 transform transition-all duration-500 hover:scale-105">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-4">
          Your Resume sharing Journey Begins Here
        </h2>
        <p className="text-gray-600 mb-6">
          It looks like you haven't shared any resume yet. 
        ...
        <button 
          ...
          onClick={() => navigate('/resumeTemplates')}
        >
          <FaPlus className="group-hover:rotate-180 transition-transform" />
          <span>Share Your First Resume Template</span>
        </button>
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-white text-opacity-80 text-lg">
              Create, customize, and manage your professional resume collection
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-auto min-w-[600px]">
                ...
                <tbody className="divide-y divide-gray-200">
                  {templates.slice(0, visibleTemplates).map((res) => (
                    <tr key={res._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-4 whitespace-nowrap">{res.company}</td>
                      <td className="px-4 py-4 whitespace-nowrap">{res.position}</td>
                      <td className="px-4 py-4 whitespace-nowrap">{res.numberOfLikes}</td>
                      <td className="px-4 py-4 whitespace-nowrap">{res.numberOfDislikes}</td>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete this resume template? 
