...
export default function InterviewCommentSection({ expId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/backend/interviewComments/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

        body: JSON.stringify({ content: comment, expId, userId: currentUser._id }),
      });
  ...
  useEffect(() => {
    const getComments = async () => {
      try {
        const res = await fetch(`/backend/interviewComments/getComments/${expId}`);
  ...
  const handleLike = async (commentId) => {
    try {
      ...
      const res = await fetch(`/backend/interviewComments/likeComment/${commentId}`, {
        method: 'PUT',
      });
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      ...
      const res = await fetch(`/backend/interviewComments/deleteComment/${commentId}`, {
        method: 'DELETE',
      });
    ...
    <div>
      {currentUser ? (
        <div className="mb-4 flex items-center gap-2 text-sm text-[#78716C]">
          ...
          <Link to="/dashboard?tab=profile" className={`text-xs font-semibold text-[#2C241B] hover:underline ${focusRing}`}>
