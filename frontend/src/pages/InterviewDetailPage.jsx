...
function FloatingElements() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      ...
      <div className="absolute top-1/4 -left-32 w-64 h-64 bg-gradient-to-br from-purple-400/15 to-pink-500/15 rounded-full blur-2xl animate-pulse" style={{animationDelay: '2s'}}></div>
      <div className="absolute bottom-1/4 right-1/4 w-48 h-48 bg-gradient-to-br from-emerald-400/20 to-cyan-500/20 rounded-full blur-2xl animate-pulse" style={{animationDelay: '4s'}}></div>
      
      {/* Floating tech icons */}
      <div className="absolute top-1/6 left-1/12 animate-bounce" style={{animationDelay: '1s', animationDuration: '3s'}}>
        <Code className="w-8 h-8 text-blue-300/40" />
      </div>
      <div className="absolute top-1/3 right-1/6 animate-bounce" style={{animationDelay: '2s', animationDuration: '4s'}}>
        <Brain className="w-6 h-6 text-indigo-300/40" />
      </div>
      <div className="absolute bottom-1/3 left-1/4 animate-bounce" style={{animationDelay: '0.5s', animationDuration: '3.5s'}}>
        <Trophy className="w-7 h-7 text-purple-300/40" />
      </div>
      <div className="absolute bottom-1/4 right-1/12 animate-bounce" style={{animationDelay: '3s', animationDuration: '2.5s'}}>
        <Zap className="w-6 h-6 text-yellow-300/40" />
      </div>
      <div className="absolute top-1/2 left-1/8 animate-bounce" style={{animationDelay: '1.5s', animationDuration: '4s'}}>
        <Globe className="w-8 h-8 text-cyan-300/40" />
      </div>
      <div className="absolute top-3/4 right-1/3 animate-bounce" style={{animationDelay: '2.5s', animationDuration: '3s'}}>
        <Layers className="w-6 h-6 text-emerald-300/40" />
      </div>
      
      {/* Geometric shapes */}
      <div className="absolute top-1/5 right-1/5 w-4 h-4 bg-blue-200/30 rotate-45 animate-spin" style={{animationDuration: '8s'}}></div>
      <div className="absolute bottom-1/5 left-1/6 w-6 h-6 bg-purple-200/30 rotate-45 animate-spin" style={{animationDuration: '12s', animationDirection: 'reverse'}}></div>
      <div className="absolute top-2/3 left-1/5 w-3 h-3 bg-indigo-200/40 rounded-full animate-ping" style={{animationDelay: '3s'}}></div>
...
function BackgroundPattern() {
  return (
    <div className="fixed inset-0 z-0">
      ...
      <div className="absolute inset-0 bg-gradient-to-tl from-purple-50/50 via-transparent to-cyan-50/30"></div>
      
      {/* Radial gradient overlay */}
      <div className="absolute inset-0 bg-radial-gradient from-white/40 via-transparent to-transparent"></div>
            ...
            <div className="flex-1 p-6 sm:p-8 lg:p-12 bg-gradient-to-bl from-white/90 to-slate-50/40 backdrop-blur-sm">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-slate-200/30 shadow-lg hover:shadow-xl transition-shadow duration-300">
                  ...
                  <p className="text-sm text-slate-600 bg-slate-50/60 px-3 py-2 rounded-lg">
                    {(experience.rating || 3) <= 2 ? '🟢 Beginner Friendly' :
                     (experience.rating || 3) <= 3 ? '🟡 Moderate Difficulty' :
                     (experience.rating || 3) <= 4 ? '🟠 Challenging' : '🔴 Expert Level'}
                  </p>
                </div>
