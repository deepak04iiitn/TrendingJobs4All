...
import { fetchSalaries, deleteSalary } from '../../lib/admin-api';
...
export default function AdminSalaryStructures() {
  ...
  const fetcher = useCallback(async ({ page, limit, search: q }) => {
    const data = await fetchSalaries({ page, limit, search: q });
  ...
  useEffect(() => {
    if (error) toast.error('Failed to load salary structures');
  }, [error]);
  ...
  return (
    <div>
      <AdminSectionHeader
        ...
        description="Review and remove community-submitted salary structures."
      />
      ...
      <AdminDataTable
        ...
        columns={[
          ...
          { key: 'ctc', label: 'CTC', render: (r) => r.ctc || '—' },
          {
            key: 'createdAt',
            label: 'Date',
